...
import type { Question, AssessmentResponse } from "@lifesync/types";
...
export default function QuizPage() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // SDK now handles fallback internally!
      ...
      if (result.is_offline) {
        ...
        const pendingItem = {
          ...
          responses: Array.from(responses.entries()), // Save as array of tuples
          timestamp: Date.now(),
          result: result
        };

        const existingQueue = JSON.parse(localStorage.getItem("pending_sync_queue") || "[]");
        existingQueue.push(pendingItem);
        localStorage.setItem("pending_sync_queue", JSON.stringify(existingQueue));
      }
      ...
      <main className="min-h-screen flex items-center justify-center p-6">
        ...
        >
          <Card className="text-center">
            ...
            {/* DEBUG INFO */}
            <div className="bg-black/30 p-4 rounded mb-4 text-left text-xs font-mono text-yellow-300 overflow-auto">
              <p>API URL: {process.env.NEXT_PUBLIC_API_URL || 'UNDEFINED'}</p>
              <p>Env Check: {process.env.NODE_ENV}</p>
            </div>
